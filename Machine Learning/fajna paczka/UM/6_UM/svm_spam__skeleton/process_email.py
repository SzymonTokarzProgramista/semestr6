...
def process_email(email_contents: str) -> List[int]:
    ...
    # FIXME: Load the vocabulary. #fixed
    vocabulary_dict = get_vocabulary_dict()

    # FIXME: Initialize the return value. #fixed
    word_indices = []
    ...
    # FIXME: Convert email content to lower case. #fixed
    ...
    # FIXME: Handle numbers. #fixed
    # Convert all sequences of digits (0-9) to a 'number' token.
    email_contents = re.sub('[0-9]+', 'number', email_contents) #mozna tez '\d+'

    # FIXME: Handle URLs. #fixed
