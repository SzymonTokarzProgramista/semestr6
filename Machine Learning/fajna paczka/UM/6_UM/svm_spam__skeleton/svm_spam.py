...
# FIXME: Create a linear SVC classifier (with C = 0.1). #fixed
clf = svm.SVC(kernel='linear', C=0.1, random_state=1, probability=True)

# FIXME: Fit the SVC model using the training data. #fixed
clf.fit(X_train, y_train)

# FIXME: Predict the labelling. #fixed
y_pred = clf.predict(X_train)

# FIXME: Compute the training accuracy. #fixed
...
# FIXME: Load the test dataset ('data/spamTest_X.csv', 'data/spamTest_y.csv'). #fixed
...
# FIXME: Print the list of 15 most prominent features. #fixed
...
idx = np.argsort(-weights) #malejaco posortowane

vocabulary_dict = get_vocabulary_dict()

print('\nTop predictors of spam: \n')
for i in range(15):
    # FIXME: Replace each `None` with an appropriate expression. #fixed
